        ...
        requests.getRequest('fba/test/webhookCall?order_id=1722').then(data => {
            if (data.success) {
                ...
                if (data.data.customer) {
                    this.state.client_details.customer_name = data.data.customer.first_name + "  " + data.data.customer.last_name
                    this.state.client_details.contact_email = data.data.customer.email
                    this.state.client_details.contact_number = data.data.billing_address.phone
                }
                if (data.data.line_items) {
                    this.state.order.number_of_line_item = Object.keys(data.data.line_items).length
